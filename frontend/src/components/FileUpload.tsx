...
export default function FileUpload({ onUpload }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      setUploaded({ name: file.name, docId: result.doc_id });
      ...
      {uploaded ? (
        <div className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-emerald-400 truncate">{uploaded.name}</p>
            <p className="text-xs text-gray-400 font-mono truncate">{uploaded.docId}</p>
          </div>
          <button
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition ${
            dragging ? "border-emerald-400 bg-emerald-500/10" : "border-gray-700 hover:border-gray-500"
          }`}
        >
          {uploading ? (
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-sm font-medium text-gray-300">Drop PDF here or click to browse</p>
              <p className="text-xs text-gray-500">Annual reports, 10-K, earnings, bank statements</p>
            </div>
          )}
        </div>
      )}
